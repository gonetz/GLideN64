		...
		if (_glinfo.isGLES2) {
			...
			if (config.generalEmulation.enableLOD == 0) {
				...
				if (config.texture.bilinearMode == BILINEAR_3POINT)
					...
					"#define READ_TEX_MIPMAP(name, tex, texCoord, lod, mode)											\\\n"
					"  {																								\\\n"
					"  mediump vec2 texSize = vec2(textureSize(tex, int(lod)));											\\\n"
					"  mediump vec2 offset = fract(texCoord*texSize - vec2(0.5));										\\\n"
					"  offset -= step(1.0, offset.x + offset.y);														\\\n"
					"  lowp vec4 c0 = TEX_OFFSET_##mode(offset, tex, texCoord, lod);									\\\n"
					"  lowp vec4 c1 = TEX_OFFSET_##mode(vec2(offset.x - sign(offset.x), offset.y), tex, texCoord, lod);	\\\n"
					"  lowp vec4 c2 = TEX_OFFSET_##mode(vec2(offset.x, offset.y - sign(offset.y)), tex, texCoord, lod);	\\\n"
					"  name = c0 + abs(offset.x)*(c1-c0) + abs(offset.y)*(c2-c0); 										\\\n"
					"  }																								\n"
					;
				else
					...
					"#define READ_TEX_MIPMAP(name, tex, texCoord, lod, mode) name = TEX_FETCH_##mode(tex, texCoord, lod)	\n"
					;
				m_part +=
					"uniform lowp int uEnableLod;		\n"
					"uniform mediump float uMinLod;		\n"
					...
					"																		\n"
					"mediump float mipmap(out lowp vec4 readtex0, out lowp vec4 readtex1) {	\n"
					"  READ_TEX_MIPMAP(readtex0, uTex0, vTexCoord0, 0.0, NORMAL);			\n"
					"  READ_TEX_MIPMAP(readtex1, uTex1, vTexCoord1, 0.0, MIPMAP);			\n"
					"																		\n"
					"  mediump float fMaxTile = float(uMaxTile);							\n"
					"  mediump vec2 dx = abs(dFdx(vLodTexCoord));							\n"
					"  dx *= uScreenScale;													\n"
					...
					"  READ_TEX_MIPMAP(lodT, uTex1, vTexCoord1, lod_tile, MIPMAP);			\n"
					"  READ_TEX_MIPMAP(lodT_m1, uTex1, vTexCoord1, lod_tile_m1, MIPMAP);	\n"
					"  READ_TEX_MIPMAP(lodT_p1, uTex1, vTexCoord1, lod_tile + 1.0, MIPMAP);	\n"
					"  if (lod_tile < 1.0) {												\n"
					"    if (magnify) {														\n"
					//     !sharpen && !detail
					"      if (uTextureDetail == 0) readtex1 = readtex0;					\n"
