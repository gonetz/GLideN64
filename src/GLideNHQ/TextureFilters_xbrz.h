...
enum class ColorFormat //from high bits -> low bits, 8 bit per channel
{
    ...
    ABGR, //including alpha channel, BGRA byte order on little-endian machines
    ABGR_UNBUFFERED, //like BGR, but without the one-time buffer creation overhead (ca. 100 - 300 ms) at the expense of a slightly slower scaling time
};
